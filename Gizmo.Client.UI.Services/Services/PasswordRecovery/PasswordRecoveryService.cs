...
public sealed class PasswordRecoveryService : IPasswordRecoveryService
{
    ...
    {
        var model = new GetRecoveryMethodsModel
        {
            Value = value,
            ValueKind = PasswordRecoveryStartRequestMapper.ToValueKind(identifierKind)
        };
        var raw = await _recoveriesClient.GetMethodsAsync(model, ct);
