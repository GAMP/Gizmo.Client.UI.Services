...
namespace Gizmo.Client.UI.View.Services
{
    ...
    public sealed class ClientServerCartViewService : CartViewServiceBase<ClientServerCartViewState>
    {
        ...
        public async Task<bool> SetNote(string? note, CancellationToken cancellationToken = default)
        {
            ...
            {
                await _notificationService.ShowAlertNotification(AlertTypes.Danger, _localizationService.GetString("GIZ_GEN_AN_ERROR_HAS_OCCURRED"), ex.Message);
        ...
        private async ValueTask InvalidateCartAsync(WebApiClientException webApiException, CancellationToken cancellationToken = default)
        {
            ...
            if (webApiException.ErrorCodeType == (int?)ExceptionCode.Cart && webApiException.ErrorCode == (int)CartErrorCode.InvalidCartId)
            {
                ...
                await _notificationService.ShowAlertNotification(AlertTypes.Danger, _localizationService.GetString("GIZ_GEN_AN_ERROR_HAS_OCCURRED"), webApiException.Message);
        ...
        protected override async ValueTask HandleRequestErrorAsync(Exception exception, ICartRequest request, CartRequestContext cartRequestContext, CancellationToken cancellationToken = default)
        {
            ...
            if (exception is WebApiClientException webApiException)
            {
                ...
                {
                    await _notificationService.ShowAlertNotification(AlertTypes.Danger, _localizationService.GetString("GIZ_GEN_AN_ERROR_HAS_OCCURRED"), _assemblyResourcesLocalizationService.GetLocalizedStringValue((CartErrorCode)webApiException.ErrorCode));
                }
                else if (webApiException.ErrorCode.HasValue && webApiException.ErrorCodeType == (int)ExceptionCode.Promotion)
                {
                    await _notificationService.ShowAlertNotification(AlertTypes.Danger, _localizationService.GetString("GIZ_GEN_AN_ERROR_HAS_OCCURRED"), _assemblyResourcesLocalizationService.GetLocalizedStringValue((PromotionErrorCode)webApiException.ErrorCode));
                }
                else
                {
