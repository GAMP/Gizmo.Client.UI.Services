    ...
    public sealed class ClientServerCartViewService : CartViewServiceBase<ClientServerCartViewState>
    {
        ...
        protected override async ValueTask HandleRequestErrorAsync(Exception exception, ICartRequest request, CartRequestContext cartRequestContext, CancellationToken cancellationToken = default)
        {
            ...
            if (exception is WebApiClientException webApiException)
            {
                ...
                {
                    await _notificationService.ShowAlertNotification(AlertTypes.Danger, _localizationService.GetString("GIZ_GEN_AN_ERROR_HAS_OCCURED"), _assemblyResourcesLocalizationService.GetLocalizedStringValue((CartErrorCode)webApiException.ErrorCode));
                }
                else if (webApiException.ErrorCode.HasValue && webApiException.ErrorCodeType == (int)ExceptionCode.Promotion)
                {
                    await _notificationService.ShowAlertNotification(AlertTypes.Danger, _localizationService.GetString("GIZ_GEN_AN_ERROR_HAS_OCCURED"), _assemblyResourcesLocalizationService.GetLocalizedStringValue((PromotionErrorCode)webApiException.ErrorCode));
                }
                else
                {
